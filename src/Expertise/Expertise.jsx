...
function Expertise() {
    ...
    const handleClick = (index) => {
        if (toggle) {
            setServices(services.map((service) => (service.index === index ? {
                ...service, visibility: 'visible', opacity: '1', position: 'relative',
            } : service)));
            setToggle(!toggle);
        } else {
            setServices(services.map((service) => (service.index === index ? {
                ...service, visibility: 'hidden', opacity: '0', position: 'absolute',
            } : service)));
            setToggle(!toggle);
        }
    };
    ...
    return (
        <div id="expertise">
            <h2>I don&rdquo;t predict the Future, We create it</h2>
